...
const StartIndexingStep = ({ selectedRepos, repos, repoLabels, isIndexing, indexingStarted, onStartIndexing }: StartIndexingStepProps) => (
  <div className="w-96 bg-white rounded shadow p-6 flex flex-col items-center">
    ...
    <ul className="mb-4 w-full">
      {selectedRepos.map(repoId => {
        ...
        return (
          <li key={repoId} className="mb-2">
            <span className="font-medium">{repo?.name}</span>
            <span className="ml-2 text-gray-500">{repoLabels[repoId]}</span>
          </li>
        )
      })}
    </ul>
